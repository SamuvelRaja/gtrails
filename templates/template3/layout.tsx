...
export default async function ClinicLayout({ children, params }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800 selection:bg-emerald-900 selection:text-white scroll-smooth flex flex-col">
      {/* Top Info Bar */}
      <div className="bg-emerald-900 text-gray-300 py-3 px-8 text-[13px] border-b border-emerald-950 relative z-50">
      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-8 h-24 flex justify-between items-center">
          <Link href={basePath} className="flex items-center gap-4 leading-none">
            <div className="w-12 h-12 bg-gray-50 rounded-full flex items-center justify-center text-emerald-900 border border-gray-200">
              <Stethoscope className="w-6 h-6" />
            </div>
            <h1 className="text-xl font-medium tracking-tight text-emerald-900">
              {clinic.name || 'Clinic Name'}
            </h1>
          </Link>
          
          <nav className="hidden lg:flex items-center gap-10 font-semibold text-xs text-gray-400 uppercase tracking-widest">
            <Link href={`${basePath}/services`} className="hover:text-emerald-900 transition-colors">Services</Link>
            <Link href={`${basePath}/about-us`} className="hover:text-emerald-900 transition-colors">About Us</Link>
            <Link href={`${basePath}/gallery`} className="hover:text-emerald-900 transition-colors">Gallery</Link>
            <Link href={`${basePath}/patient-guides`} className="hover:text-emerald-900 transition-colors">Guides</Link>
            <Link href={`${basePath}/contact-us`} className="hover:text-emerald-900 transition-colors">Contact</Link>
          </nav>

          <Link href={`${basePath}/contact-us`} className="hidden md:flex items-center gap-2 bg-emerald-900 text-white px-8 py-3.5 rounded-full font-medium hover:bg-emerald-950 transition-colors text-sm tracking-wide shadow-sm">
             Book Appointment
          </Link>
        </div>
      </header>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 mb-16 text-gray-500">
            <div className="lg:col-span-2 space-y-6">
              <Link href={basePath} className="flex items-center gap-4">
                ...
                <h4 className="text-xl font-medium tracking-tight text-emerald-900">{clinic.name || 'Premium Clinic'}</h4>
              </Link>
            ...
            <div>
              ...
              <ul className="space-y-4 font-light">
                <li><Link href={`${basePath}/services`} className="hover:text-emerald-900 transition-colors inline-block w-full text-[15px]">Services</Link></li>
                <li><Link href={`${basePath}/about-us`} className="hover:text-emerald-900 transition-colors inline-block w-full text-[15px]">About Us</Link></li>
                <li><Link href={`${basePath}/gallery`} className="hover:text-emerald-900 transition-colors inline-block w-full text-[15px]">Gallery</Link></li>
                <li><Link href={`${basePath}/patient-guides`} className="hover:text-emerald-900 transition-colors inline-block w-full text-[15px]">Patient Guides</Link></li>
                <li><Link href={`${basePath}/contact-us`} className="hover:text-emerald-900 transition-colors inline-block w-full text-[15px]">Contact Us</Link></li>
              </ul>
            </div>
          </div>
